...
def tsvToCAMI(argv):
	...
	try:
		opts, args = getopt.getopt(argv, "i:n:m:o:u:t:s:", [])
	...
	next(tsvOutput)# skip the first and second line
	next(tsvOutput)
	
	taxPaths = {}
	for line in tsvOutput:
		line = line.rstrip("\r\n")
		if line.startswith('#') or line == "\n" or line.startswith("unclassified"):
			continue
		line = line.split('\t')
		specID = line[4]
		quantity = float((line[8]).rstrip('%'))
