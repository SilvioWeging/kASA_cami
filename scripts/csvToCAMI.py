...
def csvToCAMI(argv):
	...
	nameFile = ''
	kValue = "12"
	uniqueOrNonUnique = 'n' #default: non-unique frequency
	threshold = 0.0
	sampleID = ""
	
	try:
		opts, args = getopt.getopt(argv, "i:n:m:o:k:u:t:s:", [])
	...
	firstLine = (next(kASAOutput)).split(',') # skip the first line
	rowIdx = 2
	for i in range(len(firstLine)):
		...
		if uniqueOrNonUnique == 'u':
			...
			if "Overall" in entry and kValue in entry and "rel. freq." in entry:
				rowIdx = i
				break
		elif uniqueOrNonUnique == "ou":
			if "Overall unique" in entry and kValue in entry and "rel. freq." in entry:
				rowIdx = i
				break
		else:
			if "Non-unique" in entry and kValue in entry and "rel. freq." in entry:
				rowIdx = i
				break
	...
	for line in kASAOutput:
		...
		line = line.split(',')
		specID = line[0]
		quantity = float(line[rowIdx])*100.0
